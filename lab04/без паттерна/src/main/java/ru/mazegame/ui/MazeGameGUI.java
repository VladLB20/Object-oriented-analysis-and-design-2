...
public class MazeGameGUI extends JFrame {
    ...
    private final DataSource dataSource;   // для прямого доступа к БД (анти-паттерн!)
    ...
    private void doMove(int dx, int dy) {
        ...
        if (result.getEncounteredEnemy().isPresent()) {
            ...
            if (battleResult == BattleDialog.BattleResult.PLAYER_WON) {
                // Удаляем врага из списка и из БД напрямую
                enemies.remove(enemy);
                try (Connection conn = dataSource.getConnection();
                     PreparedStatement stmt = conn.prepareStatement("DELETE FROM enemy WHERE id = ?")) {
                ...
                worldService.updatePlayerPosition(player);  // метод теперь public (см. сервис)
            }
    ...
    // Панель рисования – точно такая же, как в варианте со шлюзами.
